...
const RU: Record<string, string> = {
	...
	"Transcribe & summarize — {prompt}": "Транскрибировать и законспектировать — {prompt}",
	...
	"{action} failed: {message}. See AI tasks for details.": "Не удалось выполнить «{action}»: {message}. Подробнее на панели «Задачи ИИ».",
	"Open error log": "Открыть журнал ошибок",
	"Copy last error to clipboard": "Скопировать последнюю ошибку в буфер обмена",
	"Error log": "Журнал ошибок",
	"No errors recorded": "Ошибок нет",
	"Copy all": "Скопировать всё",
	"Clear log": "Очистить журнал",
	"All errors copied to clipboard": "Все ошибки скопированы в буфер обмена",
	"Copy error": "Скопировать ошибку",
	"Error copied to clipboard": "Текст ошибки скопирован в буфер обмена",
	Failed: "Ошибка",
	Retry: "Повторить",
	Dismiss: "Скрыть",
	...
	"Default summary prompt": "Промпт конспекта по умолчанию",
	"Used for recordings, commands, and note summaries. The file context menu lets you choose any prompt by name.":
		"Используется для записей, команд и конспектов заметок. В контекстном меню файла можно выбрать любой промпт по названию.",
	"Summary prompt {number}": "Промпт конспекта {number}",
	"Name this prompt for the context menu, then enter the instructions sent to the language model.":
		"Задайте название промпта для контекстного меню, затем введите инструкции для языковой модели.",
	"Prompt name": "Название промпта",
	"Untitled prompt": "Промпт без названия",
	"New prompt": "Новый промпт",
	"Add summary prompt": "Добавить промпт конспекта",
